                ...
                cards.length > 0 && cards.map((card, i) => {
                    return (
                        <Card active={selectedCard?.card_id === card?.card_id ? true : false} key={i} onClick={() => setSelectedCard(card)} >
                            <CardLeft>
                                <Text fontSize="16px" letterSpacing="3px" fontWeight="700" marginBottom="10px">{card?.number}</Text>
                                <Flex>
                                    <Text fontSize="12px" letterSpacing="1.5px" fontWeight="400" marginRight="30px">{card?.name}</Text>
                            ...
                            {selectedCard?.card_id === card.card_id && <CheckImageWrapper>
