...
const Table = ({ tableTop, data, showCheckbox, showPagination,filterColumns, perPageRows,selectedItems, getSelectedItems }: TableProps) => {
...
useEffect(()=>{
  ...
  const handleCheckboxClick = (event:React.ChangeEvent<HTMLInputElement>,selected?:unknown,id?:'header') => {
  ...
  return (
    <>
                  ...
                  {Object.values(row).map((cellData: any, idx: number) => (
                    <>
                      {showCheckbox && idx === 0 && 
                        <Checkbox   
                            label={""} 
                            onChange={(e:React.ChangeEvent<HTMLInputElement>)=>handleCheckboxClick(e,cellData,undefined)}
      ...
      {showPagination && <CustomPagination
        ...
        labelRowsPerPage={<Input 
                            placeholder={'e.g 5'} 
                            onChange={(val)=>val!==0?setRowsPerPage(val):setRowsPerPage(5)} 
